...
class sys_tablesController extends Controller {
  ...
  public function listar() {
    ...
    if (Validate::integer('listar', true) == 1) {
      ...
      $ids = $_POST['selected'];
      $opt == 4 ? $this->model->deleteTablas($ids) : $this->model->updateTablas($opt, $ids);
    }
  ...
  public function insertar() {
    ...
    if (Validate::integer('actualizar', true) == 1) {
      ...
      $fields['UseFileM'] = isset($fields['UseFileM']) ? $fields['UseFileM'] : 0;
      $this->model->updateTable($fields);
      Url::redirect($this->baseUrl . 'listar');
    }

    $data['tables_add'] = $this->model->getAllTables();
    $data['list_tables'] = $this->model->showTables();
    $this->view->assign('data', $data);
    $this->view->render();
  }
  ...
  public function editar($id) {
    ...
    $data[$this->tabla] = $this->model->getTable($id);
    $data['tables_add'] = $this->model->getAllTables($id);
    $data['list_tables'] = $this->model->showTables();
