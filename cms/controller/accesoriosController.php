...
class accesoriosController extends Controller {
  ...
  public function listar() {
    ...
    if (Validate::integer('listar', true) == 1) {
      ...
      if ($opt == 4) {
        $this->model->deleteAccesorios($ids);
      }
      if ($opt == 8) {
        $this->duplicar($ids);
      }
  ...
  public function insertar() {
    ...
    if (Validate::integer('actualizar', true) == 1) {
      ...
      $data['caracter'] = isset($_POST['caracter']) ? $_POST['caracter'] : [];
      ...
      $id = $this->model->updateAccesorio($fields);
  ...
  public function editar($id) {
    ...
    if (Validate::integer('actualizar', true) == 1) {
      ...
      $fields['UsuarioID'] = Session::get('user_id');
      //
      $this->model->updateAccesorio($fields);
      $this->model->updateMeta($data['meta'], $id);
      $this->model->updateGaleria($data['galeria'], $id);
    ...
    $data[$this->tabla] = $this->model->getAccesorio($id);
    $data['categorias'] = $this->model->getCategorias();
    $data['monedas'] = $this->model->getMonedas();
  ...
  public function addActions($row) {
    $urlProd = $this->model->getURLAccesorio($row['ID']);
  ...
  public function duplicar($ids) {
    $propId = $ids[0];
    $newId = $this->model->duplicateAccesorio($propId);
...
//    echo $caption . '<br><br>';
  ...
  public function upload() {
    echo $this->uploadImage($_FILES['userfile'], $this->config['PIAncho'], $this->config['PIAlto'], $this->config['PTAncho'], $this->config['PTAlto'], true);
  }
  ...
  public function delimage() {
    ...
    if ($id) {
      echo $this->model->deleteItemGaleria($id);
    }
