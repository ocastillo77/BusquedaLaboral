...
class sys_formsController extends Controller {
  ...
  public function listar() {
    ...
    if (Validate::integer('listar', true) == 1) {
      ...
      $ids = $_POST['selected'];
      $opt == 4 ? $this->model->deleteBanners($ids) : $this->model->updateForms($opt, $ids);
    }
  ...
  public function insertar() {
    ...
    if (Validate::integer('actualizar', true) == 1) {
      ...
      if (empty($nombre)) {
        $data['alert'] = Helper::alertMessage('danger', 'Debe ingresar el Nombre del Formulario');
        $this->view->assign('data', $data);
        $this->view->render();
        exit;
      }

      $this->model->updateForm($fields);
      Url::redirect($this->baseUrl . 'listar');
    }

    $data['sel_tablas'] = $this->model->getTablas();
    $this->view->assign('data', $data);
    $this->view->render();
  }
  ...
  public function editar($id) {
    ...
    $data[$this->tabla] = $this->model->getForm($id);
    $data['sel_tablas'] = $this->model->getTablas();
  ...
  public function addActions($row) {
    ...
    $urlElems = URL_CMS . 'sys_objects/listar/' . $row['ID'];

    $icon1 = Helper::tag2('i', '', ['class' => 'fa fa-edit']);
    $icon2 = Helper::tag2('i', '', ['class' => 'fa fa-list']);

    $link1 = Helper::tag2('a', $icon1 . ' Editar', ['class' => 'btn btn-info btn-sm', 'href' => $urlEdit]);
    $link2 = Helper::tag2('a', $icon2 . ' Elementos', ['class' => 'btn btn-primary btn-sm', 'href' => $urlElems]);
