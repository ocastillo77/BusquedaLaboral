...
function alertMessage(style, message) {
	...
	if (message !== '') {
		...
		$('#message').delay(800).slideDown('medium');
...
function alertMessageNew(obj, type = '', message = '') {
	var css = 'alert-' + type;

	if (message !== '') {
		$('#' + obj).addClass(css);
		$('#' + obj).delay(800).slideDown('medium');
	...
	$('#' + obj).html('<span>' + message + '</span>');
}

function showAlertMessage(obj) {
	$('#' + obj).delay(800).slideDown('medium');
	$('body,html').animate({ scrollTop: 0 }, 800);
...
function uploadFile() {
	var checkExt = new RegExp(/^(pdf|docx)$/);
	var urlUpload = $('#urlUploadFile').val();
	var urlIcon = $('#urlIcon').val();
	var code = 'file';
	var button = $('#btn-' + code);
			...
			if (isJson) {
				switch (data.success) {
					case 0:
						...
						break;
					case 1:
						setFile(code, data.name, data.url, urlIcon);
						break;
				}
				$('#ldr-' + code).hide();
				this.enable();
...
function setFile(code, name, url, urlIcon) {
	$('#' + code).val(name);
	$('#img-' + code).removeAttr('scr');
	$('#img-' + code).attr('src', urlIcon);
...
$(document).ready(function () {
	...
	if ($('#urlUploadFile').length > 0) {
		uploadFile();
	}

	if ($('#alert-message').length > 0 && $('#alert-message').attr('data-show') == 1) {
		showAlertMessage('alert-message');
	}
});

function changeNivel(value) {
	if (value == 1) {
		$('#label-nivel').html('¿Cuáles?');
	} else {
		$('#label-nivel').html('¿A qué se debe?');
	}
}

function changeGrupo(value) {
	if (value == 3) {
		$('#box-hijos').removeClass('d-none');
	} else {
		$('#box-hijos').addClass('d-none');
	}
}

function changeDispo(value) {
	if (value == 3) {
		$('#box-horario').removeClass('d-none');
	} else {
		$('#box-horario').addClass('d-none');
	}
}
...
function changeProvincia(id) {
	var urlDeptos = $("#config").attr("data-urldeptos");
...
/* MERCADO PAGO */
$(document).ready(function () {
	console.log($("#urlMPago").length)
	if ($("#link-mpago").length > 0 && $("#urlMPago").length > 0) {
		var urlMPago = $("#urlMPago").val();
		console.log(urlMPago)
		...
		$.ajax({
			...
			success: function (data) {
				if (data != "") {
					createCheckoutButton(data.publicKey, data.preferenceId);
				}
			},
		});
	}
...
function createCheckoutButton(publicKey, preferenceId) {
...
$('.form-ajax').submit(function (e) {
	...
	var sendingText = form.data('sending-text') || 'Enviando...';
	...
	$.ajax({
		...
		success: function (response) {
			if (response.code === 200) {
				...
				alertMessageNew('alert-form', 'success', response.message);
				...
				alertMessageNew('alert-form', 'danger', response.message);
			}
		},
		error: function () {
			button.prop('disabled', false).text(originalText);
			alertMessageNew('alert-form', 'danger', 'Ocurrió un error inesperado.');
		}
	});
});
...
$(document).ready(function () {
	...
	$('.toggle-password').click(function () {
		...
		$(this).html(passwordFieldType === 'password' ? '<i class="fa fa-eye-slash"></i>' : '<i class="fa fa-eye"></i>');
	});
});

Inputmask("(999) 999-9999").mask("#celular");
Inputmask("(999) 999-9999").mask("#telefonoE");

function validateForm(bloque) {
	var valid = true;

	$('#formBloque' + bloque + ' .form-control-test').each(function () {
